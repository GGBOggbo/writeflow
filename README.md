...
- **联网搜索增强**：选题阶段先由 AI 理解搜索意图，再走 wxrank 历史优先路由，后续阶段自动复用搜索上下文
...
WXRANK_DEEP_DIVE_LIMIT=4
WXRANK_COMMENT_ARTICLE_LIMIT=2
...
| 阶段 | 联网搜索 | 说明 |
|------|---------|------|
| 选题 | 用户可选 | AI 规划搜索意图，历史库优先，实时搜索兜底 |
| 提纲 | 自动复用 | 复用选题搜索结果 |
| 大纲 | 自动复用 | 复用选题搜索结果 |
| 正文 | 永远断网 | 保护成稿真实感 |
| 标题摘要 | 自动复用 | 复用选题搜索结果 |
...
接口调用次数、产品积分与人民币成本的统一口径见 [API 调用与成本账本](docs/api-cost-ledger.md)。当前默认路由每开启一次联网选题，wxrank 成本约为 ¥0.05～¥0.26；实时兜底结果不足 4 篇时，最低可能为 ¥0.13。

搜索意图解析与选题生成属于同一个服务端操作：选题阶段首次成功生成免费，同一工作流同一阶段重新生成成功后扣 0.05 项目积分，不会因为内部多了一次 AI 规划调用而重复扣费。规划失败时会使用保留版本号和产品名的规则计划降级；最终没有合格搜索结果时，系统会直接进行基础选题生成，不把跑题资料注入 prompt。
