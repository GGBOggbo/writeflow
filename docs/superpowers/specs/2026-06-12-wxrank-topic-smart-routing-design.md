...
1. 从现有选题搜索 query 中拆出“历史宽词”“实时精确词”和“本地评分词”。
2. 调用 wxrank `artlist` 查询当前月份，使用历史宽词。
3. 对结果执行发布时间过滤、关键词相关性评分、链接去重和无效数据剔除。
4. 若合格结果不足 5 篇，再调用上一个自然月的 `artlist`。
5. 合并两次历史结果，仅保留最近 30 天内的文章。
6. 若合格历史文章达到 5 篇，停止搜索，不调用实时接口。
7. 若仍不足 5 篇，使用实时精确词调用 `getso`。
8. 合并历史与实时结果，按链接去重并排序，最多向上层返回 8 篇。

“历史库有结果”不能只判断接口数组非空。只有经过质量过滤后至少保留 5 篇，才视为历史池命中。

## 查询词适配

wxrank 不能机械复用极致了的搜索 query。真实接口测试表明，将“痛点”直接拼进 `getso` 会把结果误匹配为“融资痛点”，导致公众号文章完全跑题。

查询词分成三组：

- 历史宽词：选取最能代表主题实体或领域的 1-2 个核心词，用于 `artlist` 扩大召回。
- 实时精确词：保留主题实体、对象和事件词，但移除“痛点、现状、结构、大纲、人群、最新”等业务提示词，用于 `getso`。
- 本地评分词：保留主题中的全部有效核心词，用于对历史结果进行标题和正文相关性评分。

示例：

```text
原始 query：Claude神话模型 痛点
历史宽词：Claude
实时精确词：Claude神话模型
本地评分词：Claude、神话、模型
```

2026 年 6 月 12 日实测中，`artlist("Claude")` 返回 50 篇，本地过滤后有 13 篇高度相关结果；`getso("Claude神话模型")` 返回 20 篇高度相关结果；`getso("Claude神话模型 痛点")` 则严重跑偏。因此查询词适配是替换接口的必要条件，不属于可选优化。
...
为保持当前正文阶段对标效果，参考池生成后默认选择最多 4 篇标杆文章，但完全改用 wxrank：

1. `artlist` 已自带阅读、点赞、在看和分享数据，不再调用 `getrk`。
2. 对选中的最多 4 篇调用 `artinfo`，获得完整 HTML、长链接和 `comment_id`。
3. 对拥有 `comment_id` 的标杆文章调用 `getcm`，默认最多处理 4 篇，可由 `WXRANK_DEEP_DIVE_LIMIT` 调整。
4. 留言按置顶和点赞数排序，只保留现有配置要求的前 N 条。
5. 正文阶段将最多 4 篇完整正文全部压缩为独立 `benchmarkSummary`，并将 4 份摘要全部交给正文模型参考，不在中间阶段再次截断为 2 篇。
...
| 场景 | 调用 | 预计积分 |
| --- | --- | ---: |
| 当前月历史池直接命中 | `artlist` 1 次 + `artinfo` 4 次 + `getcm` 4 次 | 25 |
| 补查上月后命中 | `artlist` 2 次 + `artinfo` 4 次 + `getcm` 4 次 | 26 |
| 历史不足，实时兜底 | 上述调用 + `getso` 1 次 | 35-36 |
