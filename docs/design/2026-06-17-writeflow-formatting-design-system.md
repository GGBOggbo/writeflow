...
(注:以下纪律来自 open-design 设计系统审计——stripe/xiaohongshu 的 tokens.css 实证)
...
### 4.0 Elevation 分级纪律(来自 open-design 审计)

open-design 的 token-schema 把 elevation 分 4 级,Writeflow 采纳这套分级,
**禁止一个模块同时用多种 elevation**(避免视觉混乱):

| 级 | 含义 | CSS | 用在哪类模块 |
|---|---|---|---|
| **flat** | 平,无深度 | 无 | 正文段、内联引用 |
| **ring** | 1px 描边环 | `box-shadow:0 0 0 1px border` | 卡片型(已用于 wf-points/wf-metric) |
| **raised** | 浮起(低语阴影) | `box-shadow:0 2px 8px rgba(...)` | 强调型(浮出正文) |
| **ring-accent** | 品牌色描边环 | `box-shadow:0 0 0 1px accent` | 主锚点(编号/标签) |

**补充纪律(open-design 实证)**:
- 小红书:"depth comes from **rounding and spacing**, not shadows" — 能用圆角+间距制造卡片感,就不用阴影
- 小红书:"one accent per screen, **never two**" — 强调色克制到极致
- stripe:elevation 是"单一最具品牌辨识度的 token",但 Writeflow 受微信约束,优先 ring 而非 raised
