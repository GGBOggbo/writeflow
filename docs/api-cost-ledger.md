# API 调用与成本账本

> 最近核对：2026-06-24
>
> 计算依据：当前代码默认配置，以及 wxrank 价格说明。供应商调价或调用链变化后需要重新核对。

## 一眼看懂

这张表记录的是：**生成选题时，每开启一次联网搜索所产生的 wxrank 成本。** 后续 Brief、提纲、正文和标题摘要会复用搜索资料，不会再次请求 wxrank。

| 场景 | 产品扣分 | wxrank 请求数 | wxrank 实际成本 |
|---|---:|---:|---:|
| 选题，不开启联网搜索 | 首次 0 / 重新生成 0.05 项目积分 | 0 | ¥0.00 |
| 选题，当月历史命中 | 首次 0 / 重新生成 0.05 项目积分 | 5～7 | ¥0.05～¥0.15 |
| 选题，补查上月命中 | 首次 0 / 重新生成 0.05 项目积分 | 6～8 | ¥0.06～¥0.16 |
| 选题，实时搜索兜底 | 首次 0 / 重新生成 0.05 项目积分 | 4～9 | ¥0.13～¥0.26 |
| Brief、提纲、正文、标题摘要 | 各阶段首次 0 / 重新生成 0.05 项目积分 | 0 次新增 wxrank 请求 | ¥0.00 新增 wxrank 成本 |

“项目积分”是产品内生成额度，不等于 ¥1，也不等于 wxrank 的账户余额。

## wxrank 当前单价

| 接口 | 用途 | 单价 | 当前流程 |
|---|---|---:|---|
| `artlist` | 获取公众号每日爆文列表 | ¥0.01/次 | 使用 |
| `getso` | 实时获取搜一搜文章列表 | ¥0.10/次 | 历史结果不足时使用 |
| `artinfo` | 获取公众号文章内容 | ¥0.01/次 | 最多 4 次 |
| `getcm` | 获取公众号文章留言 | ¥0.05/次 | 最多 2 次 |
| `score` | 获取当前剩余积分 | 免费 | 当前业务流程未调用 |
| `getrk` | 获取文章阅读数据 | ¥0.02/次 | 当前流程未调用 |
| `getps` | 获取公众号推文列表 | ¥0.05/次 | 当前流程未调用 |
| `getsu` | 获取公众号搜索列表 | ¥0.10/次 | 当前流程未调用 |
| `getinfo` | 根据 biz 获取公众号原始 ID | ¥0.05/次 | 当前流程未调用 |
| `getbiz` | 根据 biz 获取公众号基本信息 | ¥0.05/次 | 当前流程未调用 |
| `getpc` | 获取公众号推文列表（短链） | ¥0.05/次 | 当前流程未调用 |

## 公众号选题成本明细

当前默认配置：

- 合格历史结果阈值：5 篇。
- 最终参考结果上限：8 篇。
- `artinfo` 深拆上限：4 篇。
- `getcm` 留言文章上限：2 篇。
- 每篇最多保留 10 条留言。

| 搜索路径 | 调用组成 | 请求数 | 成本计算 | 成本范围 |
|---|---|---:|---|---:|
| 当月历史命中 | `artlist × 1 + artinfo × 4 + getcm × 0～2` | 5～7 | `0.01 + 4×0.01 + (0～2)×0.05` | ¥0.05～¥0.15 |
| 补查上月命中 | `artlist × 2 + artinfo × 4 + getcm × 0～2` | 6～8 | `2×0.01 + 4×0.01 + (0～2)×0.05` | ¥0.06～¥0.16 |
| 实时搜索兜底 | `artlist × 2 + getso × 1 + artinfo × 1～4 + getcm × 0～2` | 4～9 | `2×0.01 + 0.10 + (1～4)×0.01 + (0～2)×0.05` | ¥0.13～¥0.26 |

说明：

- `getcm` 只有在深拆文章存在 `comment_id` 时才会调用。
- 实时兜底最终可能不足 4 篇，因此 `artinfo` 可能只有 1～4 次。
- 表格按接口请求次数计算；供应商是否对特定失败请求收费，以 wxrank 实际账单为准。

## 产品积分

项目积分是产品内生成额度，不等于人民币成本。`topics`、`brief`、`outline`、`draft`、`meta` 五个阶段按 `userId + workflowId + stage` 计费：每个阶段首次成功生成免费，同一工作流同一阶段重新生成成功后扣 0.05 项目积分。AI 排版和 AI 补充素材免费；失败请求退款；系统内部自动重试不额外扣费。

| 阶段 | 产品扣分 |
|---|---:|
| 生成选题 | 首次 0 / 重新生成 0.05 |
| 生成 Brief | 首次 0 / 重新生成 0.05 |
| 生成提纲 | 首次 0 / 重新生成 0.05 |
| 生成正文 | 首次 0 / 重新生成 0.05 |
| 生成标题与摘要 | 首次 0 / 重新生成 0.05 |

生成失败会退回预扣积分。选题内部的搜索意图规划和标杆总结属于同一次产品操作，不额外扣用户项目积分，但仍会产生真实 AI Token 成本。

完整走完五个生成阶段的首次生成不消耗项目积分；重新生成才按阶段扣 0.05 项目积分。项目积分只表示用户产品额度，不代表项目的实际人民币成本。

## AI 模型成本

AI 人民币成本取决于输入资料长度、输出长度和重试次数，不能用一个固定金额表示。

统一计算公式：

`AI 成本 = 输入 Token ÷ 1,000,000 × 输入单价 + 输出 Token ÷ 1,000,000 × 输出单价`

| AI 请求 | 何时发生 | 产品是否另扣积分 | 人民币成本 |
|---|---|---|---|
| 搜索意图规划 | 开启选题联网搜索时 | 否，包含在选题操作中 | 按 Token 计算 |
| 标杆文章总结 | 有合格搜索资料时 | 否，包含在选题操作中 | 按 Token 计算 |
| 生成选题 | 点击生成选题时 | 首次 0 / 重新生成 0.05 项目积分 | 按 Token 计算 |
| 生成 Brief | 点击生成 Brief 时 | 首次 0 / 重新生成 0.05 项目积分 | 按 Token 计算 |
| 生成提纲 | 确认 Brief 后 | 首次 0 / 重新生成 0.05 项目积分 | 按 Token 计算 |
| 生成正文 | 确认提纲后 | 首次 0 / 重新生成 0.05 项目积分 | 按 Token 计算 |
| 生成标题与摘要 | 正文完成后 | 首次 0 / 重新生成 0.05 项目积分 | 按 Token 计算 |

项目尚未记录每次 AI 请求的输入、输出 Token，因此此处不填写猜测的固定金额。

## 维护检查表

修改下列内容时同步更新本账本：

- `WXRANK_HISTORY_MIN_RESULTS`
- `WXRANK_RESULT_LIMIT`
- `WXRANK_DEEP_DIVE_LIMIT`
- `WXRANK_COMMENT_ARTICLE_LIMIT`
- wxrank 接口单价
- wxrank 搜索路由与重试逻辑
- 产品计费阶段或单次项目积分
- AI 模型和供应商价格

历史设计文档中按 `getcm × 4` 得出的 ¥0.25、¥0.26、¥0.35～¥0.36 已不符合当前代码。当前代码默认最多调用 `getcm × 2`，应以本账本为准。
